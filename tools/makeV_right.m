...
if k < 0
	...
	V = [ -(s+2*B)  		(-2*B - s)*(4*B*K + 2*L*s + 3*s*u + 4*B*H*k*K*s*u + 2*H*k*L*s^2*u + 2*H*k*s^2*u^2); ...
	       (s+2*B)   		(2*B + s)*(2*B*K + L*s + 4*B*H*k*K*s*u + 2*H*k*L*s^2*u + 2*H*k*s^2*u^2);  ...
	      -2*u*s*k       -4*k*s*u*(2*B*K + L*s + s*u)*(1 + H*k*s*u); ...
